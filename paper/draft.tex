...
The trailer class, absent from the Singapore validation split, accounts for
$-0.157$ of the total gap; excluding trailer the 9-class mAP gap is $-4.75$
($-10.4\%$ relative), confirming a genuine visual-domain contribution.
...
$13\%$ larger than within-Boston ($0.110$ vs.\ $0.097$).
...
A plausible explanation is that Singapore has a different vehicle-type mix than Boston
(e.g., smaller taxis, different bus dimensions), so the per-class size priors the model
has internalized from Boston training do not transfer, causing systematic scale
miscalibration rather than pure appearance confusion.
...
likely reflecting denser urban footfall in Singapore-OneNorth's downtown district.
...
\caption{Layer-wise representation similarity.
  ...
  Cosine similarity computed per-camera over full spatial feature maps;
  debiased CKA (Kornblith et al.~\citeyear{kornblith2019cka}, Supp.) applied
  after PCA-100\,D (70.6\,\% variance, \texttt{img\_feat}; 68.1\,\%, \texttt{bev\_embed}).
  Within-Boston cosine from CAM\_FRONT 8$\times$8 pooled features (consistent with
  CKA basis); cross-city cosine from per-camera spatial method.
  $^{\dagger}$2000 bootstrap iterations, 80\,\% subsampling without replacement.}
\label{tab:drift}
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{@{}lccccc@{}}
\toprule
Comparison & Cos.\,$\mu$ & Cos.\,dist & CKA (debiased) & 95\,\%\,CI$^{\dagger}$ \\
\midrule
\multicolumn{5}{@{}l}{\textit{After FPN (\texttt{img\_feat})}} \\
\quad Cross-city Boston$\to$Singapore & 0.424 & 0.576 & 0.003 & $[-0.001,\ 0.007]$ \\
\quad Within-Boston (reference)       & --    & --    & 0.001 & $[-0.006,\ 0.009]$ \\
\midrule
\multicolumn{5}{@{}l}{\textit{After BEV encoder (\texttt{bev\_embed})}} \\
\quad Cross-city Boston$\to$Singapore & 0.890 & 0.110 & 0.009 & $[\ 0.004,\ 0.014]$ \\
\quad Within-Boston (reference)       & 0.903 & 0.097 & $-$0.003 & $[-0.010,\ 0.003]$ \\
\bottomrule
\end{tabular}
\end{table}
...
space by a metric that does not suffer from t-SNE projection artifacts.
...
positive examples; at $\tau = 0.5$ the box count would be approximately halved,
reducing coverage without addressing the structural failure identified below.
