...
95\,\% CI $[1.101,\ 1.167]$, permutation $p < 0.001$).
...
\caption{Layer-wise representation similarity.
  ...
  \texttt{bev\_embed} cosine: CAM\_FRONT 8$\times$8 pooled features for both
  cross-city and within-Boston (consistent method; both give 0.890 cross-city).
  \texttt{img\_feat} cross-city cosine (0.424): per-camera full-spatial method.
  \texttt{img\_feat} within-Boston cosine$^{\ddagger}$: CAM\_FRONT 8$\times$8
  pooled (not directly comparable to 0.424 for ratio computation; the
  method-consistent img\_feat ratio is $0.307/0.279 = 1.098\times$, 95\,\% CI
  $[1.064,\ 1.135]$, $p{<}0.001$).
...
\quad Within-Boston (reference)       & $0.721^{\ddagger}$ & $0.279^{\ddagger}$ & 0.001 & $[-0.006,\ 0.009]$ \\
\midrule
...
(95\,\% CI $[1.101,\ 1.167]$, permutation $p < 0.001$; both computed with the
same CAM\_FRONT 8$\times$8 pooled extraction, 2000 bootstrap iterations).
City-level separation is therefore confirmed in the original high-dimensional feature
space by a metric defined in the original space and statistically characterized,
without relying on t-SNE projection distances.
...
\begin{enumerate}
  ...
  \item \textbf{The BEV encoder normalizes appearance but not structural geometry.}
    ...
    1.167]$, $p < 0.001$) and debiased CKA of $0.009$ (95\,\% CI $[0.004,\ 0.014]$)
